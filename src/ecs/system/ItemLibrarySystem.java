...
//        System.out.println("Name for 1302000 " + miip.names.get(miip.items.get(1302000)).name);
    ...
    /**
     * Regular preprocessing of item ID whenever some component of an Equip wants to be checked. Need to check if it's generated
     * already by generateEquip. If not, generate and return associated entityId.
     * @param itemId ItemId to check
     * @return EntityID corresponding to the ItemId
     */
    private int equipCheck(int itemId) {
        if (!Equip.isEquip(itemId) || !exists(itemId))
            return -1;

        int entityId = entity(itemId);
        if (equipStats.get(entityId) == null)
            generateEquip(itemId);
...
//        incRMAI       incHP       incM
    ...
    public String getName(int itemId) {
        Name name = getProperty(Name.class, itemId);
        return name == null ? "NO-NAME" : name.name;
    }
    ...
    public short getSlotMax(int itemId) {
        // TODO: think about handling for specific jobs that increase slot amount on usables such as stars/bullets
        if (!exists(itemId) || Equip.isEquip(itemId))
            return 1;

        int entityId = entity(itemId);
        SlotMax stat = getProperty(SlotMax.class, itemId);
        if (stat == null) {
            stat = slotMax.create(entityId);
            MapleData item = getItemData(itemId);
            if (item != null) {
                try {
                    MapleData smEntry = item.getChildByPath("info/slotMax");
                    stat.slotMax = (short) MapleDataTool.getInt(smEntry);
                } catch (NullPointerException e) {
                    stat.slotMax = 100;
                }
            }
...
//                        break;
//                    default:
//                        for (Entry<String, Integer> stat : properties.entrySet()) {
//                            switch (stat.getKey()) {
//                                case "STR":
...
//                    nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
//                }
//                if (Randomizer.nextInt(99) < properties.get("cursed")) {
//                    return null;
//                }
//            }
//        }
...
//    }
//
//    //default, all the equip properties.
//    public Equip randomizeStats(Equip equip) {
//        MapleStat[] properties = {
//            MapleStat.INT,
//            MapleStat.STR,
//            MapleStat.LUK,
//            MapleStat.DEX,
...
//            MapleBuffStat.WDEF};
//
//        return randomizeStats(equip, properties, buffStats);
//    }
//
//    public Equip randomizeStats(Equip equip,
//            MapleStat[] properties, MapleBuffStat[] buffStats) {
//
//        equip = randomizeStats(equip, properties);
//        equip = randomizeStats(equip, buffStats);
//        return equip;
//    }
//
//    public Equip randomizeStats(Equip equip, MapleStat[] properties) {
//        for (MapleStat stat : properties) {
//            switch (stat) {
//                case HP:
//                case MP:
