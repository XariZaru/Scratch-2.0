...
public class WorldInformation extends Component {
	...
	private void encodeChannels(OutboundPacket mplew) {
		...
		for (Channel ch : channels) {
			...
			mplew.writeMapleAsciiString((ScratchConstants.WORLD_NAMES[world] + "-" + (x + 1)));
	...
	public int populationStatus() {
		...
		if (connected >= ScratchConstants.GAME_SERVER_LOAD)
			return 2;
		else if (connected >= ScratchConstants.GAME_SERVER_LOAD * .8)
			return 1;
		return 0;
	}
